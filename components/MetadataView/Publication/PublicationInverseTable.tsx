...
export default function PublicationInverseTable({
  resourceID = '',
}: {
  resourceID: string;
}) {
  ...
  if (error) {
    console.log('Publicatio Inverse table error: ' + error);
    return null;
  }
  ...
  return (
    <div style={{ width: '100%' }} className="basic-inside-content-div">
      <div className="flex flex-col items-start gap-6 p-6 self-stretch rounded-xl bg-[#ffffff]">
        <h5>Associated Collections and Resources</h5>
        <input
          type="text"
          placeholder="Search…"
          value={search}
